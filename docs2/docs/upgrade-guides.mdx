...
Background: CPGQL was previously based on tinkerpop gremlin (accessible via `.raw`), and is now based on [OverflowDb Traversal](https://github.com/ShiftLeftSecurity/overflowdb/blob/master/traversal/src/main/scala/overflowdb/traversal/Traversal.scala) instead. The main drivers behind this change are better performance, less complexity and less dependencies. OverflowDb Traversal is a Scala collection with additional graph steps, which means we now inherit many useful steps from the rich Scala standard collection library, where previously we'd need to convert between Traversal and Scala collection. 
At the same time, Traversal offers largely the same steps (and semantics) as it's Tinkerpop predecessor. 
