...
import io.shiftleft.codepropertygraph.generated.nodes.{Call, Literal, Method, Return}
...
class MethodReturnTests extends RubyCode2CpgFixture(withDataFlow = true) {
  ...
  "implict RETURN node for RubyCallWithBlock" in {
    val cpg = code("""
        | def foo &block
        |  puts block.call
        | end
    ...
    inside(cpg.method.name("bar").l) {
      case bar :: Nil =>
        inside(bar.block.astChildren.collectAll[Method].l) {
