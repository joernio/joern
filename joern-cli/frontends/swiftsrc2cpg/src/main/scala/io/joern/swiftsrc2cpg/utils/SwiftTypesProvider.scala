...
case class SwiftTypesProvider(config: Config, parsedSwiftInvocations: Seq[Seq[String]]) {
  ...
  /** Converts a BufferedReader to a parallel sequence of JSON objects.
    *
    * This method reads lines from the BufferedReader, filters for lines that start with '{' (indicating JSON objects),
    * and returns them as a parallel sequence.
    *
    * @param reader
    *   The BufferedReader to read from
    * @return
    *   A parallel sequence of JSON strings
    */
