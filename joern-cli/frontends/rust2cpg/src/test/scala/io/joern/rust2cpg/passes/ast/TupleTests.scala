...
class TupleTests extends Rust2CpgSuite(noSysRoot = true) {
  ...
  "a 2-tuple literal" should {
    ...
    "lower to a tupleLiteral call" in {
      inside(cpg.call.nameExact(RustOperators.tupleLiteral).l) { case tuple :: Nil =>
        tuple.code shouldBe "(1, 2)"
        tuple.methodFullName shouldBe RustOperators.tupleLiteral
        tuple.dispatchType shouldBe DispatchTypes.STATIC_DISPATCH
      }
    }

    "type the tupleLiteral as (i32, i32)" in {
      cpg.call.nameExact(RustOperators.tupleLiteral).typeFullName.l shouldBe List("(i32, i32)")
    }

    "have the i32 literals as arguments" in {
      inside(cpg.call.nameExact(RustOperators.tupleLiteral).argument.l) { case (a: Literal) :: (b: Literal) :: Nil =>
        a.code shouldBe "1"
  ...
  "a 3-tuple literal of mixed types" should {
    ...
    "type the tupleLiteral with the element types" in {
      cpg.call.nameExact(RustOperators.tupleLiteral).typeFullName.l shouldBe List("(i32, &str, bool)")
    }

    "have the i32 literal as the first argument" in {
      inside(cpg.call.nameExact(RustOperators.tupleLiteral).argument(1).l) { case (i: Literal) :: Nil =>
        i.code shouldBe "1"
    ...
    "have the &str literal as the second argument" in {
      inside(cpg.call.nameExact(RustOperators.tupleLiteral).argument(2).l) { case (s: Literal) :: Nil =>
        s.code shouldBe "\"x\""
    ...
    "have the bool literal as the third argument" in {
      inside(cpg.call.nameExact(RustOperators.tupleLiteral).argument(3).l) { case (b: Literal) :: Nil =>
  ...
  "a nested tuple literal" should {
    ...
    "lower as two tupleLiteral calls" in {
      cpg.call.nameExact(RustOperators.tupleLiteral).code.toSet shouldBe Set("(1, 2)", "((1, 2), 3)")
    }

    "type the outer tupleLiteral as ((i32, i32), i32)" in {
      cpg.call.nameExact(RustOperators.tupleLiteral).codeExact("((1, 2), 3)").typeFullName.l shouldBe List(
        "((i32, i32), i32)"
      )
    }

    "type the inner tupleLiteral as (i32, i32)" in {
      cpg.call.nameExact(RustOperators.tupleLiteral).codeExact("(1, 2)").typeFullName.l shouldBe List("(i32, i32)")
    }

    "have the inner tupleLiteral as the first argument of the outer" in {
      inside(cpg.call.nameExact(RustOperators.tupleLiteral).codeExact("((1, 2), 3)").argument(1).l) {
        case (inner: Call) :: Nil =>
          inner.code shouldBe "(1, 2)"
          inner.methodFullName shouldBe RustOperators.tupleLiteral
      }
    }

    "have the i32 literal as the second argument of the outer" in {
      inside(cpg.call.nameExact(RustOperators.tupleLiteral).codeExact("((1, 2), 3)").argument(2).l) {
        case (rest: Literal) :: Nil =>
