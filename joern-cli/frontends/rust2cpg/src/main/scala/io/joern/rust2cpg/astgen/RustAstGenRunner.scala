...
class RustAstGenRunner(config: Config) extends AstGenRunner(RustAstGenRunner.astGenMetaData, config) {
  ...
  override def skippedFiles(in: Path, astGenOut: List[String]): List[String] = {
    // NB: rust_ast_gen does not support skipping files yet.
    // TODO: extract "Skipped: <path>" entries from astGenOut. These correspond to files that failed
    //  parsing in rust_ast_gen.
    List.empty
  }
