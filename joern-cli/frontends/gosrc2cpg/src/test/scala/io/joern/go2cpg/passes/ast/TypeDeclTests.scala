...
class TypeDeclTests extends GoCodeToCpgSuite {
  ...
  "when structure declared with multiple argument" should {
    ...
    "test argument of structure declaration node(call node)" in {
      val List(widthNode, heightNode) = cpg.call.nameExact("Rectangle").argument.isIdentifier.l
      widthNode.name shouldBe "width"
      heightNode.name shouldBe "height"
  ...
  "when is imported from other package structure" should {
    val cpg = code(
      """
        package main
        |import (
        |    "path/to/package" // Replace with the actual package path
        |)
        |func main() {
        |    rect := package.Rectangle{l: 10, w: 20}
        |}
        |""".stripMargin,
      "test.go"
    )
