...
class MethodAndTypeCacheBuilderPass(astFiles: List[String], config: Config) {
    ...
    val futures = astFiles
      .map(file => {
        Future {
          ...
          val astCreator      = new AstCreator(relPathFileName, parserResult)(config.schemaValidation)
