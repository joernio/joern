...
trait AstForGenDeclarationCreator(implicit withSchemaValidation: ValidationMode) { this: AstCreator =>
    ...
    genDecl
      ...
      .flatMap { genDeclNode =>
        genDecl.json(ParserKeys.Tok).str match
          case "import" => astForImport(genDeclNode)
          case "const"  => List[Ast]()
          case "type"   => astForTypeSpec(genDeclNode)
          case "var"    => astForValueSpec(genDeclNode)
          case _        => List[Ast]()
      }
      .toSeq
  }

  def astForTypeSpec(typeSpec: ParserNodeInfo): Seq[Ast] = {
    typeSpec.node match {
      ...
      case TypeSpec =>
        ...
        Ast.storeInDiffGraph(Ast(typeDeclNode_).withChild(Ast(modifier)), diffGraph)
  ...
  def astForImport(nodeInfo: ParserNodeInfo): Seq[Ast] = {
  ...
  def astForValueSpec(valueSpec: ParserNodeInfo): Seq[Ast] = {
