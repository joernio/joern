...
class SimpleCfgCreationPassTest extends CfgTestFixture(() => new JsCfgTestCpg()) {
  ...
  "CFG generation for simple fragments" should {
    ...
    "have correct structure for untagged runtime node in call" in {
      ...
      succOf("\"!\"") shouldBe expected(("<operator>.formatString(\"Hello \", world, \"!\")", AlwaysEdge))
      succOf("<operator>.formatString(\"Hello \", world, \"!\")") shouldBe expected(
    ...
    "have correct structure for untagged runtime node" in {
      ...
      succOf("\"\"", 1) shouldBe expected(("<operator>.formatString(\"\", x + 1, \"\")", AlwaysEdge))
      succOf("<operator>.formatString(\"\", x + 1, \"\")") shouldBe expected(("RET", AlwaysEdge))
    }
    ...
    "have correct structure for tagged runtime node" in {
      ...
      succOf("\"\\..\"") shouldBe expected(("<operator>.formatString(\"../\", 42, \"\\..\")", AlwaysEdge))
      succOf("<operator>.formatString(\"../\", 42, \"\\..\")") shouldBe expected(
        ("String.raw(<operator>.formatString(\"../\", 42, \"\\..\"))", AlwaysEdge)
      )
      succOf("String.raw(<operator>.formatString(\"../\", 42, \"\\..\"))") shouldBe expected(("RET", AlwaysEdge))
    }
