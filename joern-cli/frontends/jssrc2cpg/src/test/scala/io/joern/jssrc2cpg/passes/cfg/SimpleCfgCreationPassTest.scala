...
class SimpleCfgCreationPassTest extends CfgTestFixture(() => new JsCfgTestCpg()) {
  ...
  private def testForInOrOf()(implicit cpg: Cpg): Unit = {
    ...
    succOf("arr") shouldBe expected(("Runtime.iterator(arr)", AlwaysEdge))
    succOf("Runtime.iterator(arr)") shouldBe expected(("_iterator_0 = Runtime.iterator(arr)", AlwaysEdge))
    succOf("_iterator_0 = Runtime.iterator(arr)") shouldBe expected(("_result_0", AlwaysEdge))
