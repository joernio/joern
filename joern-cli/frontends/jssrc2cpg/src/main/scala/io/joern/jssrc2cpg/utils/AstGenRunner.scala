...
class AstGenRunner(config: Config) {
  ...
  private def processEjsFiles(in: File, out: File, ejsFiles: List[String]): Try[Seq[String]] = {
    ...
    val result = ExternalCommand.run(s"$astGenCommand$executableArgs -t ts -i . -o $out", out.toString())
  ...
  private def vueFiles(in: File, out: File): Try[Seq[String]] = {
    ...
    if (files.nonEmpty)
      ExternalCommand.run(s"$astGenCommand$executableArgs -t vue -i . -o $out", in.toString())
    else Success(Seq.empty)
  }

  private def jsFiles(in: File, out: File): Try[Seq[String]] =
    ExternalCommand.run(s"$astGenCommand$executableArgs -t ts -i . -o $out", in.toString())
