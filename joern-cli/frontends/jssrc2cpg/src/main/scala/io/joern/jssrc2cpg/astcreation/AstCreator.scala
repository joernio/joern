...
class AstCreator(
  ...
  protected def astForNode(json: Value): Ast = createBabelNodeInfo(json) match {
    ...
    case jsxSpreadChild @ BabelNodeInfo(BabelAst.JSXSpreadChild)           => astForJsxExprContainer(jsxSpreadChild)
    case jsxSpreadAttr @ BabelNodeInfo(BabelAst.JSXSpreadAttribute)        => astForJsxSpreadAttribute(jsxSpreadAttr)
