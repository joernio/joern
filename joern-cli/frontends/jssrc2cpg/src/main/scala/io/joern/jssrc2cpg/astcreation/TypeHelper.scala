...
trait TypeHelper {
  ...
  private def typeForFlowType(flowType: BabelNodeInfo): String = {
    flowType.node match {
      ...
      case BabelAst.ThisTypeAnnotation          => dynamicInstanceTypeStack.headOption.getOrElse(Defines.ANY.label)
  ...
  private def typeForTsType(tsType: BabelNodeInfo): String = {
    tsType.node match {
      ...
      case BabelAst.TSThisType          => dynamicInstanceTypeStack.headOption.getOrElse(Defines.ANY.label)
      ...
      case BabelAst.TSOptionalType      => typeForTypeAnnotation(createBabelNodeInfo(tsType.json(TYPE_ANNOTATION_KEY)))
      case BabelAst.TSRestType          => typeForTypeAnnotation(createBabelNodeInfo(tsType.json(TYPE_ANNOTATION_KEY)))
      ...
      case BabelAst.TSParenthesizedType => typeForTypeAnnotation(createBabelNodeInfo(tsType.json(TYPE_ANNOTATION_KEY)))
  ...
  protected def typeFor(node: BabelNodeInfo): String = {
    if (hasKey(node.json, TYPE_ANNOTATION_KEY)) {
      val tpe = typeForTypeAnnotation(createBabelNodeInfo(node.json(TYPE_ANNOTATION_KEY)))
      registerType(tpe, tpe)
      tpe
    } else if (hasKey(node.json, RETURN_TYPE_KEY)) {
      val tpe = typeForTypeAnnotation(createBabelNodeInfo(node.json(RETURN_TYPE_KEY)))
      registerType(tpe, tpe)
      tpe
    } else {
