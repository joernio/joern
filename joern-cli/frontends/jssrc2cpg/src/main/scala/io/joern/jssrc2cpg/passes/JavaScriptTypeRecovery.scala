      ...
      matchingExports.flatMap { exp =>
        exp.argument.l match {
          ...
          case ::(x: Call, ::(b: MethodRef, _)) =>
            ...
            symbolTable.append(LocalVar(alias), b.referencedMethod.start.astParent.collectAll[Method].fullName.toSet)
          case ::(_, ::(y: Call, _)) =>
      ...
      case "<operator>.new" =>
        c.astChildren.l match {
          case ::(fa: Call, ::(i: Identifier, _)) if fa.name == Operators.fieldAccess =>
            symbolTable.append(
              c,
              visitIdentifierAssignedToFieldLoad(i,fa.asInstanceOf[FieldAccess]).map(t => s"$t$pathSep$ConstructorMethodName")
            )
          case _ => Set.empty
        }
