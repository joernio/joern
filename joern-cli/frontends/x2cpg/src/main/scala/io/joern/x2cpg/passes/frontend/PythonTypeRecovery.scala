...
class PythonTypeRecovery(cpg: Cpg) extends CpgPass(cpg) {
  ...
  override def run(builder: DiffGraphBuilder): Unit =
    try {
      ...
      def importCalls = cpg.call.nameExact("import")
    ...
    private def setImports(importCall: Call): Unit = {
      importCall.argumentOut.l match {
        case List(path: Literal, funcOrModule: Literal) =>
          ...
          symbolTable.put(funcOrModule, calleeNames)
        case List(path: Literal, funcOrModule: Literal, alias: Literal) =>
          ...
          symbolTable.put(alias, calleeNames)
  ...
  /** Using an entry from the symbol table, will queue the CPG modification to persist the recovered type information.
    ...
    */
  class PersistRecoveredTypeTask(builder: DiffGraphBuilder, sbKey: SBKey, typeHints: Set[String])
      extends RecursiveTask[Unit] {
    override def compute(): Unit = {
      val SBKey(filename, _) = sbKey
      cpg.file
        .nameExact(filename)
        .method
        .ast
        .foreach {
          case x: Local if sbKey.equals(x) =>
            builder.setNodeProperty(x, PropertyNames.DYNAMIC_TYPE_HINT_FULL_NAME, typeHints.toSeq)
          case x: Identifier if sbKey.equals(x) =>
  ...
  object SBKey {
    def fromNode(cfgNode: CfgNode): SBKey = {
      val name = cfgNode match {
        case x: Identifier => x.name
        case x: Method     => x.name
        case x: Call       => x.name
        case x             => x.code
      }
      val filename = cfgNode.file.name.headOption match {
        case Some(fileName) => fileName
        case None =>
          logger.warn(
            s"Unable to successfully use file name for symbol table, type recovery may become more imprecise. Node: ${cfgNode.propertiesMap()}"
          ); ""
      }
      SBKey(filename, name)
    }
  ...
  private class SymbolTable {
    ...
    def apply(node: CfgNode): Set[String] = table(fromNode(node))

    def put(sbKey: SBKey, typeFullNames: Set[String]): Option[Set[String]] =
      table.put(sbKey, typeFullNames)

    def put(node: CfgNode, typeFullNames: Set[String]): Option[Set[String]] =
      put(fromNode(node), typeFullNames)

    def append(node: CfgNode, typeFullName: String): Option[Set[String]] =
      append(node, Set(typeFullName))

    def append(node: CfgNode, typeFullNames: Set[String]): Option[Set[String]] =
    ...
    def contains(node: CfgNode): Boolean = contains(fromNode(node))

    private def get(sbKey: SBKey): Set[String] = table.getOrElse(sbKey, Set.empty)

    def get(node: CfgNode): Set[String] = get(fromNode(node))
