...
class AstCreator(
  ...
  private def addImportsToScope(compilationUnit: CompilationUnit): Seq[NewImport] = {
    ...
    val specificImportNodes                = specificImports.map { importStmt =>
      ...
      val importNode = newImportNode(getImportCode(importStmt), typeFullName, name, importStmt)
    ...
    val asteriskImportNodes = asteriskImports match {
      case imp :: Nil =>
        ...
        val importNode   = newImportNode(getImportCode(imp), typeFullName, name, imp)
          .isWildcard(true)
