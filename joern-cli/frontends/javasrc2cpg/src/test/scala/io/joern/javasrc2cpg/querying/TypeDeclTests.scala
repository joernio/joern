...
class TypeDeclTests extends JavaSrcCodeToCpgFixture {
  ...
  "should contain a type decl for `foo` with correct fields" in {
    ...
    x.inheritsFromTypeFullName should contain theSameElementsAs  List("a.b.c.d.Woo", "java.lang.Object")
  ...
  "should contain type decl for external type `int`" in {
    ...
    x.inheritsFromTypeFullName.toList shouldBe List("java.lang.Object")
    x.aliasTypeFullName shouldBe None
    x.order shouldBe -1
  ...
  "should create a correct type decl for inner interface" in {
    cpg.typeDecl.nameExact("InnerInterface").l match {
      case List(interface) =>
        interface.fullName shouldBe "Foo.OuterClass.InnerInterface"
        ...
        interface.method.toList match {
          case List(method) =>
            ...
            method.fullName shouldBe "Foo.OuterClass.InnerInterface.id:int(int)"
  ...
  "should create type decl for inner class implementing interface" in {
    ...
    cpg.typeDecl.name("InnerClass").l match {
      case List(innerClass) =>
        innerClass.fullName shouldBe "Foo.OuterClass.InnerClass"
        innerClass.inheritsFromTypeFullName should contain theSameElementsAs List("java.lang.Object", "Foo.OuterClass.InnerInterface")
        innerClass.isExternal shouldBe false

        innerClass.method.nameExact("id").l match {
          case List(method) =>
            method.fullName shouldBe "Foo.OuterClass$InnerClass.id:int(int)"
            method.signature shouldBe "int(int)"
