...
class AstCreationPass(cpg: Cpg, forFiles: InputFiles, config: Config, report: Report = new Report())
  ...
  private def sourceFiles: Set[String] =
    SourceFiles.determine(Set(config.inputPath), FileDefaults.SOURCE_FILE_EXTENSIONS).toSet

  private def headerFiles: Set[String] = {
    val allHeaderFiles         = SourceFiles.determine(Set(config.inputPath), FileDefaults.HEADER_FILE_EXTENSIONS).toSet
