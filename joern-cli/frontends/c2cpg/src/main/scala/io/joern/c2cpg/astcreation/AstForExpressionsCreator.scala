...
trait AstForExpressionsCreator {
  ...
  private def astForConstructorExpression(c: ICPPASTSimpleTypeConstructorExpression, argIndex: Int): Ast = {
  ...
  protected def astForExpression(expression: IASTExpression, argIndex: Int): Ast = {
    val r = expression match {
      ...
      case c: ICPPASTSimpleTypeConstructorExpression   => astForConstructorExpression(c, argIndex)
