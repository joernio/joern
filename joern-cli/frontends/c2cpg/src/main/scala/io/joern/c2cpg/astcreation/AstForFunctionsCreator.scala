...
trait AstForFunctionsCreator { this: AstCreator =>
  ...
  private def parameterInfoForVariadicParam(
    func: IASTNode,
    hasVariadicParamAlready: Boolean,
    index: Int
  ): Seq[FunctionDeclNodePass.ParameterInfo] = {
    if (!hasVariadicParamAlready && isVariadic(func)) {
      val paramName = scopeLocalUniqueName("param")
      Seq(
  ...
  private def variadicParamsForCPPFunctionFromInfo(
    ...
    val hasVariadicParamAlready = otherParams.exists(_.isVariadic)
    parameterInfoForVariadicParam(func, hasVariadicParamAlready, otherParams.size + 1)
  }
  ...
  protected def paramIsVariadic(parameter: IASTNode): Boolean = {
    parameter match {
      case p: CPPASTParameterDeclaration =>
        ...
        || Try(parameter.getParent.getRawSignature.contains(s"$paramName...")).toOption.contains(true)
      case _ => false
    }
  }
  ...
  private def parameterNodeInfo(parameter: IASTNode, paramIndex: Int): FunctionDeclNodePass.ParameterInfo = {
    val (name, codeString, tpe, variadic) = parameter match {
      ...
      case p: CPPASTParameterDeclaration =>
        ...
        val isVariadic = p.getDeclarator.declaresParameterPack() || paramIsVariadic(p)
        val rawCode    = code(p)
        val codeString = if (isVariadic && !rawCode.contains("...")) { s"$rawCode..." }
        else { rawCode }
        (paramName, codeString, tpe, isVariadic)
