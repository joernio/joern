...
class ControlStructureTests extends JimpleCode2CpgFixture {
  ...
  "try-with-resource blocks" should {
    ...
    "create nodes for resources" in {
      cpg.method.name("foo").dotCfg.foreach(println)
  ...
  "foreach loops over arrays imported through static imports" should {
    val cpg = code("""
        |import static a.Bar.STATIC_ARR;
        |
        |public class Foo {
      ...
      .moreCode(
        """package a;
          ...
          |}
          |""".stripMargin, "Bar.java"
      )

    "create the correct number of STATIC_ARR identifiers" in {
      // One in definition in class Bar
      // One in fieldAccess STATIC_ARR.length
      cpg.fieldAccess.fieldIdentifier.canonicalName("STATIC_ARR").size shouldBe 2
      // One in indexAccess STATIC_ARR[tmpIdx] via l0 temp variable
    }
