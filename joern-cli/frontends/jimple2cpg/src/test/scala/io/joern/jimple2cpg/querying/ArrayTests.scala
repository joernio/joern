...
class ArrayTests extends JimpleCode2CpgFixture {
  ...
  "should initialize array with three address code initialization expressions" in {
    ...
    val List(placeholderArg: Identifier, arrayInit: Call) =
      m.assignment.codeExact("x = new int[3]").argument.l: @unchecked
    placeholderArg.code shouldBe "x"
    ...
    val List(stackAt0: Call, arg0: Literal) = m.assignment.codeExact("x[0] = 0").argument.l: @unchecked

    arg0.code shouldBe "0"
    arg0.typeFullName shouldBe "int"

    stackAt0.code shouldBe "x[0]"
    ...
    stackPointerAt0.code shouldBe "x"
    zero.code shouldBe "0"

    val List(stackAt1: Call, arg1: Literal) = m.assignment.codeExact("x[1] = 1").argument.l: @unchecked

    arg1.code shouldBe "1"
    arg1.typeFullName shouldBe "int"

    stackAt1.code shouldBe "x[1]"
    ...
    stackPointerAt1.code shouldBe "x"
    one.code shouldBe "1"

    val List(stackAt2: Call, arg2: Literal) = m.assignment.codeExact("x[2] = 2").argument.l: @unchecked

    arg2.code shouldBe "2"
    arg2.typeFullName shouldBe "int"

    stackAt2.code shouldBe "x[2]"
    ...
    stackPointerAt2.code shouldBe "x"
    two.code shouldBe "2"
  }
  ...
  "should handle arrayIndexAccesses correctly (3-address code form)" in {
    ...
    val List(indexAccess: Call, rhsStub: Identifier) = m.assignment.codeExact("x[1] = $stack2").argument.l: @unchecked
    ...
    withClue("placeholder in expr on RHS of assignment") {
      rhsStub.name shouldBe "$stack2"
      rhsStub.typeFullName shouldBe "int"
      rhsStub.code shouldBe "$stack2"
    }
  }
}
