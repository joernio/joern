  ...
  "pattern matching" should {
    "be correct" in {
      val cpg = code("""match [1, 2]:
          ...
          |    print(2)
          |""".stripMargin)
      val switch    = cpg.controlStructure.next()
      val condition = switch.astChildren.order(1).next()
      condition.code shouldBe "[1, 2]"
      condition.lineNumber shouldBe Some(1)

      val case1 = switch.astChildren.order(2).next()
      case1.label shouldBe NodeTypes.BLOCK
      case1.code shouldBe "print(1)"

      val case2 = switch.astChildren.order(3).next()
