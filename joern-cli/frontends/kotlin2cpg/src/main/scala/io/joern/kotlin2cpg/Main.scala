...
object Main extends App {
  ...
  X2Cpg.parseCommandLine(args, frontendSpecificOptions, Config()) match {
    case Some(config) =>
      if (config.inputPaths.size == 1) {
        ...
        val environment = CompilerAPI.makeEnvironment(dirsForSourcesToCompile, defaultInferenceJarPaths, Seq(), messageCollector)
  ...
  private val defaultInferenceJarPaths = Seq(
