...
import io.joern.dataflowengineoss.semanticsloader.{Parser, FullNameSemantics}
...
class DataFlowThroughLoHiRegistersTests extends GhidraBinToCpgSuite {
  ...
  val customSemantics: String =
    s""""<operator>.assignment" 2->1
       ...
       |""".stripMargin
  implicit val semantics: FullNameSemantics   = FullNameSemantics.fromList(new Parser().parse(customSemantics))
