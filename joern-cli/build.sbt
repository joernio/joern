...
Universal/mappings := (Universal/mappings).value.collect {
  case (jar, location)
      ...
      && containsModuleInfoClass(jar) =>
    val newJar = target.value / "without-module-info" / jar.getName()
    IO.copyFile(jar, newJar)
    removeModuleInfo(newJar)
    streams.value.log.info(s"workaround for scala completion bug: including a modified version of $jar without the /module-info.class entry: $newJar")
    newJar -> location
  case other =>
    other // no need to change anything
}

/** checks if the given jar contains a `/module-info.class` (only at root level) */
def containsModuleInfoClass(jar: File): Boolean = {
  ...
  val result = zipFs.getRootDirectories.asScala.exists { zipRootDir =>
    Files.list(zipRootDir).iterator.asScala.exists(_.toString == "/module-info.class")
  }
  zipFs.close()
...
/** removes `/module-info.class` from the given jar (only at root level) */
def removeModuleInfo(jar: File): Unit = {
  import java.nio.file.{Files, FileSystems}
