...
trait BridgeBase extends InteractiveShell with ScriptExecution with PluginHandling with ServerHandling {
  ...
  protected def buildRunBeforeCode(config: Config, isInteractive: Boolean = false): Seq[String] = {
    ...
    if (config.pluginToRun.contains("scan")) {
      ...
      builder += s"""Scan.defaultOpts.names = Array(${config.scanScriptNames.map(n => s""""$n"""").mkString(", ")})"""
      builder += s"""Scan.defaultOpts.tags = Array(${config.scanTagNames.map(n => s""""$n"""").mkString(", ")})"""
