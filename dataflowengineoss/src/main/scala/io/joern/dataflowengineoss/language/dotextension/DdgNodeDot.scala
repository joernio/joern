...
class DdgNodeDot(val traversal: Iterator[Method]) extends AnyVal {
  ...
  def dotDdg(implicit semantics: FullNameSemantics = DefaultSemantics()): Iterator[String] =
    DotDdgGenerator.toDotDdg(traversal)

  def dotPdg(implicit semantics: FullNameSemantics = DefaultSemantics()): Iterator[String] =
    DotPdgGenerator.toDotPdg(traversal)

  def dotCpg14(implicit semantics: FullNameSemantics = DefaultSemantics()): Iterator[String] =
    DotCpg14Generator.toDotCpg14(traversal)

  def plotDotDdg(implicit viewer: ImageViewer, semantics: FullNameSemantics = DefaultSemantics()): Unit = {
    Shared.plotAndDisplay(traversal.dotDdg.l, viewer)
  }

  def plotDotPdg(implicit viewer: ImageViewer, semantics: FullNameSemantics = DefaultSemantics()): Unit = {
    Shared.plotAndDisplay(traversal.dotPdg.l, viewer)
  }

  def plotDotCpg14(implicit viewer: ImageViewer, semantics: FullNameSemantics = DefaultSemantics()): Unit = {
