...
object Engine {
      ...
      case childNode: Expression =>
        parNode match {
          case parentNode: Expression =>
            ...
            Some(PathElement(parentNode, callSiteStack, visible, outEdgeLabel = outLabel))
  ...
  /** For a given node `node`, return all incoming reaching definition edges, unless the source node is (a) a METHOD
    ...
    * propagated to it.
    */
  private def ddgInE(node: CfgNode, path: Vector[PathElement], callSiteStack: List[Call]): Vector[Edge] = {
    node
  ...
  def deduplicate(vec: Vector[ReachableByResult]): Vector[ReachableByResult] = {
    vec
      .groupBy { x =>
        (
          x.path.headOption.map(_.node) ++ x.path.lastOption.map(_.node),
          x.resultType.isInstanceOf[PartialResult],
          x.callDepth
        )
      }
      .map { case (_, list) =>
