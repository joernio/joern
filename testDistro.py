...
@stage
def stage_joern(script_abs_dir):
    try:
...
def slice_test(script_abs_dir):
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        test_file = f"{os.path.join(script_abs_dir, "tests", "code", "javasrc", "SliceTest.java")}"
        out_file = f"{os.path.join(slice_path, "dataflow-slice-javasrc.json")}"
        slice_script = f"{os.path.join(script_abs_dir, "tests", "test-dataflow-slice.sc")}" 
