  ...
  def insecureLoadUrlToExec()(implicit engineContext: EngineContext): Query =
    Query.make(
      ...
      withStrRep({ cpg =>
        ...
        // Find WebViews with insecure loadUrl calls
        ...
        // Check if the app allows cleartext traffic
        val appUsesCleartextTraffic = cpg.appManifest.usesCleartextTraffic.nonEmpty

        // Find JavaScript interfaces exposed when cleartext traffic is allowed
        ...
        // Collect the names of exposed JavaScript interface objects
        ...
        // Check if the exposed JavaScript methods can reach exec() calls
