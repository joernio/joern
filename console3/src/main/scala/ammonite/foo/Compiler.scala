...
import ammonite.util.{Classpath, Imports, ImportData, PositionOffsetConversion, Printer}
import ammonite.compiler.internal.CompilerHelper
import dotty.ammonite.compiler.AmmCompletion
import ammonite.compiler.{AmmonitePhase, AsmPositionUpdater}
...
//                println(s"XXX1 classpath0.contains(versionsort)=${self.classPath.find(_.getPath.contains("versionsort")).isDefined}")
  ...
  def complete(
    ...
    val (start, completions) = AmmCompletion.completions(
      srcPos,
