...
class ConstrainedVersionParsingTests extends AnyWordSpec with Matchers {
  "basic semver" should {
    "parse" in {
      ConstrainedVersion.parse("1.0.0") shouldEqual Eq(SemVer("1.0.0"))
    }
  }
  
  "conjunction of semver" should {
    "parse" in {
      ConstrainedVersion.parse("!0.0.0 && <= 2.0.0 && >= 1.0.0") shouldEqual 
        And(
          And(
            Not(Eq(SemVer("0.0.0"))), 
            Lte(Eq(SemVer("2.0.0")))),
