...
object MetadataIonTextLoader {
  ...
  private def parseTypeNameInfo(reader: IonReader, packageMetadata: PackageMetadata): PackageMetadata = {
    Option(reader.next()) match
      case Some(IonType.STRUCT) => {
        ...
        reader.stepOut()
        val newPackageMetadata = packageMetadata.copy(perVersionTypeInfo = packageMetadata.perVersionTypeInfo :+ typeInfo)
        parseTypeNameInfo(reader, newPackageMetadata)
      }
      case None => packageMetadata
      case _ => throw new RuntimeException("Invalid type info package metadata format: type info list has invalid format")
  }
  ...
  private def parseTypeInfoStruct(reader: IonReader, typeMetadata: TypeMetadata = TypeMetadata()): TypeMetadata = {
    Option(reader.next()) match
      ...
      case _ => throw new RuntimeException("Invalid type info package metadata format: type info struct has invalid format")
  }
