...
/** Languages should override this providing conversions from version strings
 *  to strings usable by the fetcher. Should also provide a compare function
 *  that returns -1 for this < other, 0 this == other, 1 otherwise. The compare
 *  method is used to resolve dependencies with <,=,>,etc constraints. */
...
class SemVer(str: String) extends Version(str) {
  ...
  override def compare(other: Version): Int = {
    ...
    (major.compare(otherSemVer.major), 
      minor.compare(otherSemVer.minor), 
      patch.compare(otherSemVer.patch)) match
