...
object MavenDependencyResolver {
  ...
  private class ResolutionCandidates private (val map: Map[String, Map[SemVer2, List[DirectDependency]]]) {
    ...
    def mergeWithLoadedDeps(deps: Map[(PackageIdentifier, Version), List[DirectDependency]]): (ResolutionCandidates, Boolean) = {
  ...
  def resolveDependencies(fetcher: Fetcher, pid: PackageIdentifier, version: SemVer2): Future[List[TransitiveDependency]] = {
  ...
  private def pickAllVersions(candidates: ResolutionCandidates, 
                              constraintStack: CurrentConstraints = CurrentConstraints(),
                              pickedVersions: CurrentChoices = CurrentChoices()): CurrentChoices = {
    candidates.headOption match
      ...
      case Some((packageName, versionsToConstraints)) => {
        ...
        pickPackageVersion(packageName,
          descendingVersions,
  ...
  @tailrec
  private def pickPackageVersion(packageName: String,
  ...
  private def downloadAllTransitiveDependencies(fetcher: Fetcher, 
                                                packageNames: List[String], 
                                                candidates: ResolutionCandidates = ResolutionCandidates()): Future[ResolutionCandidates] = {
  ...
  private def downloadMetadata(fetcher: Fetcher, packageNames: List[String]): Future[Map[PackageIdentifier, Array[Byte]]] = {
    val pids = packageNames.map(PackageIdentifier(JVM, _))
    val metadatas = fetcher.fetchMetaData(pids)
    metadatas
  }
  
  private def downloadDirectDependencyInfo(fetcher: Fetcher, metadatas: Map[PackageIdentifier, Array[Byte]]): Future[Map[(PackageIdentifier, Version), Array[Byte]]] = {
    val neededVersions = listNeededVersions(metadatas)
    val directDeps = fetcher.fetchDirectDependencies(neededVersions)
    directDeps
  }
  
  private def listDirectDependencies(deps: Map[(PackageIdentifier, Version), Array[Byte]]): Map[(PackageIdentifier, Version), List[DirectDependency]] = {
  ...
  @tailrec
  private def listNeededVersions(metadatas: Map[PackageIdentifier, Array[Byte]], 
                                 versions: List[(PackageIdentifier, SemVer2)] = Nil): List[(PackageIdentifier, SemVer2)] = {
