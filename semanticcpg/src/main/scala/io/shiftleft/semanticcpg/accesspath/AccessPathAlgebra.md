...
2. the PointerShift operator, which we will write as p: <i>, and which corresponds to p+i. This again has a VariableAccess variant, such that p: <5> and p: <?> are valid paths. The property of the pointershift is that subsequent shifts collapse by addition, i.e. p: <5> <-3> <-2> * === p: *. Then, a C array access can be written as p[4] === *(p+4) === p: <4> *.
