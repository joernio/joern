...
class NodeTypeStarters(cpg: Cpg) {
  ...
  def assignment: Iterator[OpNodes.Assignment] = callsWithNameIn(allAssignmentTypes).cast[OpNodes.Assignment]

  @Doc(info = "All arithmetic operations, including shorthand assignments that perform arithmetic (e.g., '+=')")
  def arithmetic: Iterator[OpNodes.Arithmetic] = callsWithNameIn(allArithmeticTypes).cast[OpNodes.Arithmetic]

  @Doc(info = "All array accesses")
  def arrayAccess: Iterator[OpNodes.ArrayAccess] = callsWithNameIn(allArrayAccessTypes).cast[OpNodes.ArrayAccess]
