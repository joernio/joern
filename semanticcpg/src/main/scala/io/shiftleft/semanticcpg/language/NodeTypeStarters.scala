...
// TODO ensure scaladoc and @doc annotation are also generated
   ...
   * name.
   *
   * More precisely: Scala's implicit resolution reports 'ambiguous implicits' if two methods with the same name
   * but different parameters are defined in two different (implicitly reachable) classes. The `.methodRef` step is
   * defined in `generated.CpgNodeStarter`. This step (filter by name) doesn't get generated by the codegen because
   * it's more complex than the other 'filter by primary key' starter steps.
   * */
